    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <h5 class="font-bold text-center text-gray-900 text-xl">New Inventory Request</h5><br>                
                <form action="{{ route('inventory_requests.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                        @php
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="block w-96 appearance-none rounded-md border border-gray-300 px-3 py-2" onchange="toggleType()" required>
                            ...
                            <option value="2">Transfer request from another branch</option>
                        </select> 
                    </div>
                    ...
                    <div id="selectbranch" style="display:none;">
                        ...
                        <select name="branch_id" id="branch_id" class="block w-96 appearance-none rounded-md border border-gray-300 px-3 py-2">
                            ...
                            <option value="{{ $branch->id }}">{{ $branch->name }} (Manager: @if(isset($branch->manager)){{ $branch->manager->name }}@endif)</option>
                            @endforeach
                        </select> 
...
<script>
    ...
    $('#product_id').on('change', function() {
        ...
        if (productId) {
            ...
            $.ajax({
                url: '/get-branch-products/' + productId,
                type: 'GET',
