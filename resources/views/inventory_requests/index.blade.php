    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-gray-100 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <a href="{{ route('inventory_requests.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-900 focus:bg-blue-900 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:ring-offset-2 transition ease-in-out duration-150">Add Inventory Request</a><br><br>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-700 dark:text-gray-400">
                        ...
                        <tbody>                            
                            @foreach($inventory_requests as $inventory_request)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">                                    
                                ...
                                @if(isset($inventory_request->product))
                                    <img src="{{ asset('storage') }}/{{ $inventory_request->product->image }}" alt="image" width="75px" height="75px"> 
                                @endif
                                </td>
                                <td class="py-3 px-3">
                                @if(isset($inventory_request->product))
                                ...
                                Transfer <br>
                                    @if(isset($inventory_request->branch))
                                        <span class="text-xs">({{ $inventory_request->branch->name }})</span>
                                    @endif
                                @endif
                                </td>
                                <td class="py-3 px-4">
                                ...
                                <td class="py-3 px-3">
                                    ...
                                    @elseif($inventory_request->status == 4)
                                    ...
                                    @elseif($inventory_request->status == 5)
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                <form method="POST" action="{{ route('inventory_requests.destroy', 'data_id') }}">
                    @csrf
                    @method('DELETE')
